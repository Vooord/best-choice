import {
    REGISTER_START, REGISTER_SUCCESS, registerSuccess, registerFail,
...
const onRegisterStartEpic = (action$, state) => action$.pipe(
    ofType(REGISTER_START),
    mergeMap(() => {
        const {
            login, password, firstName, lastName,
        } = state.value.user.new;
...
const onRegisterSuccessEpic = action$ => action$.pipe(
