...
const occupyTopicEpic = (action$, state) => action$.pipe(
...
const updateTopicEpic = (action$, state) => action$.pipe(
    ...
    mergeMap(({ payload }) => from(
        ...
        mergeMap(() => of(updateTopicsSuccess(payload))),
...
const addTopicEpic = (action$, state) => action$.pipe(
    ofType(TOPICS_ADD),
    mergeMap(({ payload }) => from(
        ...
        mergeMap(response => of(addTopicsSuccess(response))),
        catchError(errPromise => from(errPromise)
            .pipe(
                mergeMap(err => {
                    toast.error(err.message);
...
const deleteTopicEpic = (action$, state) => action$.pipe(
    ...
    mergeMap(({ payload }) => from(
        ...
        mergeMap(() => of(deleteTopicsSuccess(payload))),
        catchError(errPromise => from(errPromise)
            .pipe(
                mergeMap(err => {
                    toast.error(err.message);
